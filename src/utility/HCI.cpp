...
String metaEventToString(LE_META_EVENT event) {
    switch (event) {
        case CONN_COMPLETE:
            return F("CONN_COMPLETE");
        case ADVERTISING_REPORT:
            return F("ADVERTISING_REPORT");
        case LONG_TERM_KEY_REQUEST:
            return F("LE_LONG_TERM_KEY_REQUEST");
        case READ_LOCAL_P256_COMPLETE:
            return F("READ_LOCAL_P256_COMPLETE");
        case GENERATE_DH_KEY_COMPLETE:
            return F("GENERATE_DH_KEY_COMPLETE");
        default:
...
String commandToString(LE_COMMAND command) {
    switch (command) {
        case ENCRYPT:
            return F("ENCRYPT");
        case LONG_TERM_KEY_REPLY:
            return F("LONG_TERM_KEY_REPLY");
        case READ_LOCAL_P256:
            return F("READ_LOCAL_P256");
        case GENERATE_DH_KEY_V1:
            return F("GENERATE_DH_KEY_V1");
        case GENERATE_DH_KEY_V2:
            return F("GENERATE_DH_KEY_V2");
        default:
            return "UNKNOWN";
    }
}

HCIClass::HCIClass() : _debug(NULL),
                       _recvIndex(0),
                       _pendingPkt(0) {
}
...
void HCIClass::poll(unsigned long timeout) {
    ...
    while (HCITransport.available()) {
        ...
        if (_recvBuffer[0] == HCI_ACLDATA_PKT) {
            ...
            bool ok_to_handle = false, packet_complete = false;
            if (_recvIndex > 3) {
                switch (_recvBuffer[1]) {
                    ...
                    case EVT_LE_META_EVENT:
                        ...
                        switch (_recvBuffer[3]) {
                            case ADVERTISING_REPORT:
                                if ((3 + _recvBuffer[2]) > 48) {
                                    if (_debug) {
                ...
                if (_recvIndex >= (3 + _recvBuffer[2])) {
                    packet_complete = true;
                }
                //ok_to_handle = true;
                if (ok_to_handle && packet_complete) {
                    if (_debug) {
                        dumpPkt("HCI EVENT RX <- ", _recvIndex, _recvBuffer);
                    }
#ifdef ARDUINO_AVR_UNO_WIFI_REV2
